...
function findSources(dict: Map<number, NodeJob>) {
...
function createDict_and_fill_empty_fields(manyNodes: Array<NodeJob>) {
	const dict = new Map()
...
function createTopoSort(manyNodes: Array<NodeJob>) {
	...
	while (dict.size > 0) {
		...
		const sortFunction = function (a: number, b: number) {
			if (dict.get(a).oriNumPreReqTo > dict.get(b).oriNumPreReqTo) {
				return -1
			}
			if (dict.get(a).oriNumPreReqTo < dict.get(b).oriNumPreReqTo) {
				return 1
